...
func (s *AccessService) SetAccess(ctx context.Context, login string, granted bool, decidedBy string) error {
	...
	// гонку всё равно не закрывает — транзакции здесь нет, — а лишний запрос добавляет.
	// Известный компромисс: заявка, поданная между GetUser и этим местом, останется pending,
	// и следующий CreateRequest её уже не перезапишет (доступ есть — короткое замыкание выше).
	// На право доступа это не влияет — источник истины has_access (Decision 3), — но
	// «повисшая» строка видна в админском списке; если это окажется важно для UX, чинить
	// нужно на уровне репозитория одной транзакцией, а не вторым чтением здесь.
