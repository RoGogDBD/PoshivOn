...
### Decision 19: `users.id` и все ссылающиеся `user_id` переведены на `utf8mb4_nopad_bin`
**Decision:** Миграция `server/migrations/005_identity_collation.up.sql` меняет сортировку
`users.id`, `user_settings.user_id`, `chats.user_id`, `access_requests.user_id` и
`calculations.user_id` с унаследованной от базы `utf8mb4_uca1400_ai_ci` на явную
`utf8mb4_nopad_bin` — единственный из доступных на MariaDB 11.4 бинарных вариантов, не
игнорирующий одновременно регистр, акценты и концевые пробелы (`utf8mb4_bin` — PAD SPACE,
проверено живьём: `'RoGogDBD' = 'RoGogDBD '` под ним всё ещё истина).
**Rationale:** [SECURITY] Задача 3 (ревью security-auditor) воспроизвела живьём на MariaDB
11.4, что унаследованная сортировка позволяет логину, отличающемуся от `RoGogDBD` или
`irina2000aleshina` только регистром, диакритикой или концевым пробелом, получить строку
администратора целиком (`role='admin'`, `has_access=1`) через `EnsureUser`/`GetUser`, а не
только не создать вторую строку, как считалось после Task 1. Тот же эффект работает и для
обычных пользователей: два разных логина, схлопывающихся друг к другу, молча делят одну
строку `users` со всеми настройками, чатами и расчётами. Прод ещё не выкатывался (Wave 12 не
запускалась), поэтому это самый дешёвый момент на исправление — решение принято
пользователем при эскалации находки, не командой реализации в одностороннем порядке.
**Alternatives considered:** (a) переезд личности на числовой `id` Яндекса вместо `login` —
отклонено как выходящее за объём Phase 1 (каскадная правка Data Models и обеих задач,
использующих `login`); (b) точечная проверка байт-в-байт в коде поверх старой сортировки
(предложено security-auditor как быстрый stop-gap) — отклонено, поскольку не закрывает саму
коллизию первичного ключа при INSERT, только сужает поверхность чтения; (c) `utf8mb4_bin` —
отклонено по итогам замера (пробельный вектор остаётся открытым).
...
| Репозиторий `RoGogDBD/PoshivOn` публичный — логины обоих администраторов из миграции `004` (`RoGogDBD`, `irina2000aleshina`) навсегда видны в истории git. Сам логин не является учётными данными (без OAuth-входа доступа не даёт), но даёт атакующему готовый список из двух целей для фишинга/social engineering вместо перебора всех авторизованных пользователей. Найдено security-аудитом Task 1, задокументировано задним числом — при написании Decision 4 этот угол не рассматривался | Принято как есть пользователем. Изменение репозитория на приватный — вне объёма фичи, отдельное решение по настройкам GitHub |
...
| Сортировка `users.id`, унаследованная от базы (`utf8mb4_uca1400_ai_ci`), регистро-, акцент- и пробелонезависима — логин, схлопывающийся к логину администратора, получал бы его строку целиком; два обычных пользователя со схлопывающимися логинами молча делили бы одну строку. Найдено security-аудитом Task 3, воспроизведено живьём | **Закрыто.** Decision 19: миграция `005_identity_collation.up.sql` переводит `users.id` и все FK `user_id` на `utf8mb4_nopad_bin`; регрессия закреплена тестом `LoginIdentityIsCaseAndSpaceSensitive` на обоих репозиториях |
