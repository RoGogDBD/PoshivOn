...
Логин первого администратора известен — `RoGogDBD`. Второй подставляется вместо
`ADMIN_LOGIN_2` перед выкаткой; способ узнать его — войти в панель и прочитать строку
`Пользователь:` (`client/src/pages/Panel.jsx:583`, значение `profile.login`).
Пока плейсхолдер не заменён, миграция создаст лишний аккаунт с логином `ADMIN_LOGIN_2`
и правами администратора — задача деплоя обязана это проверить.
...
- [ ] В выкаченной миграции нет плейсхолдера `ADMIN_LOGIN_2`; администраторов ровно два
...
- **Description:** Провести `CONTACT_EMAIL` через все пять переходов от секрета GitHub до
  ...
  воркфлоу прогона `go test ./...` и сделать его обязательным для деплоя, а также убедиться,
  что плейсхолдер второго администратора в миграции заменён на реальный логин. Нужно, потому
  что переменная, отсутствующая в allowlist `envs:`, теряется молча, тесты сегодня не
  запускаются нигде, а незаменённый плейсхолдер создаст лишний аккаунт с правами
  администратора. Результат: сервер на проде видит новую переменную, `/auth/refresh`
  доходит до бэкенда, падение тестов блокирует деплой.
- **Skill:** deploy-pipeline
- **Reviewers:** code-reviewer, security-auditor, deploy-reviewer
- **Verify-smoke:** `grep ADMIN_LOGIN_2 server/migrations/004_access_control.up.sql` → пусто; `docker exec poshivon_app env | grep CONTACT_EMAIL` → переменная присутствует; `curl -i -X POST https://<host>/auth/refresh` → ответ JSON от бэкенда, не HTML SPA; push в ветку → джоба тестов отрабатывает и блокирует деплой при падении
