...
- **Needs an explicit decision — public repo exposure.** `github.com/RoGogDBD/PoshivOn` is public (confirmed via the GitHub API: `private: false`), so the two seeded admin Yandex logins are now permanently and publicly visible in git history, and irreversibly so — the migration cannot be rewritten once applied. This is a different angle from the hardcode-vs-env-var mechanism trade-off Decision 4 settled, and it is not named in the tech-spec Risks table. Not an auth bypass (a login is not a credential), but it publishes a confirmed two-account target list for phishing. Raised by the security auditor as low severity; wants an explicit accept-and-document or a repo-visibility reconsideration, at the feature level rather than in this file.
